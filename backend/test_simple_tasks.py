#!/usr/bin/env python
"""
Simple test script to verify password reset task functions work correctly.
"""
import os
import sys
import django
...
def test_task_functions():
    """Test the password reset task functions directly."""
    print("Testing Password Reset Task Functions")
    print("=" * 50)
    
    # Import task functions directly (not as Celery tasks)
    ...
    # Test monitoring task function
    print("\n1. Testing monitoring task function...")
    try:
        # Call the function directly, not as a Celery task
        monitor_result = monitor_password_reset_token_performance.s().apply().result
    ...
    # Test token cleanup task function
    print("\n2. Testing token cleanup task function...")
    try:
        cleanup_result = cleanup_expired_password_reset_tokens.s().apply().result
    ...
    # Test attempt cleanup task function
    print("\n3. Testing attempt cleanup task function...")
    try:
        attempt_result = cleanup_old_password_reset_attempts.s(days_old=7).apply().result
